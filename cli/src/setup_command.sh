...
## Bootstrap deps + build if missing.
## Resilient to a known npm bug where ~/.npm cache contains root-owned files
## (https://github.com/npm/cli/issues/4815): falls back to a fresh tmp cache
## instead of failing.
